...
export const VideoPlayer = React.forwardRef<HTMLVideoElement, VideoPlayerProps>(({ isLive }, ref) => {
  if (isLive) {
    return (
      <div className="w-full aspect-video bg-black rounded-xl border-2 border-primary relative">
        <video ref={ref} className="w-full h-full object-cover rounded-md" autoPlay muted playsInline />
  ...
  return (
    <div className="w-full aspect-video bg-black rounded-xl flex items-center justify-center border-2 border-primary/50 relative">
       <video ref={ref} className="w-full h-full object-cover rounded-md hidden" autoPlay muted playsInline />
      <div className="text-center text-muted-foreground">
        <VideoOff className="h-12 w-12 text-primary mx-auto" />
        <p className="mt-2">Your camera feed will appear here</p>
      </div>
